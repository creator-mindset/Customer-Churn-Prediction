"""
Shopify Customer Churn & Value Intelligence App
------------------------------------------------------------
Run:
    streamlit run app.py

Requires (in the same folder):
    customer_features.csv, customer_features_scored.csv
...
(generated by feature_engineering.py + train_models.py)
------------------------------------------------------------
"""
...
# ------------------------------------------------------------------
st.set_page_config(
    page_title="Shopify Churn & Value Intelligence",
    page_icon="🛍️",
    layout="wide",
)
...
# ------------------------------------------------------------------
    ...
    if st.button("Find segment", use_container_width=True):
        # Build as a DataFrame with the exact training column names/dtype
        # (float64) so this matches what the scaler/KMeans were fit on --
        # avoids the "Buffer dtype mismatch" error from mixed dtypes.
        X_new = pd.DataFrame(
            [[float(r), float(f), float(m)]], columns=artifacts["cluster_features"]
        )
        X_new_scaled = artifacts["cluster_scaler"].transform(X_new)
        # Defensive cast: match whatever dtype the deployed KMeans model's
        # cluster centers actually are. Some environments/sklearn versions
        # end up with float32 centers even when the scaler is float64 --
        # casting here avoids a "Buffer dtype mismatch" ValueError no
        # matter which model file is actually loaded.
        X_new_scaled = X_new_scaled.astype(artifacts["kmeans"].cluster_centers_.dtype)
        seg_id = artifacts["kmeans"].predict(X_new_scaled)[0]
