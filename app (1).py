...
# ------------------------------------------------------------------
# On a fresh deploy (e.g. Streamlit Community Cloud), the trained
# .pkl / .csv artifacts may not have made it into the repo -- most
# commonly because they were pickled locally but never committed, or
# a .gitignore is excluding *.pkl / *.csv. Rather than crash with a
# FileNotFoundError, the app checks for the required files and, if
# any are missing, builds them on the spot from the raw dataset.
# Training on ~31k customers takes only a few seconds, so this is a
# safe fallback -- but for repeat deploys you should still commit the
# generated files so this step is skipped.
REQUIRED_FILES = [
    "churn_classifier.pkl", "revenue_regressor.pkl", "kmeans_model.pkl",
    "cluster_scaler.pkl", "label_encoders.pkl", "segment_name_map.pkl",
    "clf_features.pkl", "reg_features.pkl", "cluster_features.pkl",
    "customer_features_scored.csv",
]
RAW_DATA_FILE = "shopify_sales_dataset_ml_eda.csv"
...
def bootstrap_if_needed():
    ...
    if not os.path.exists(RAW_DATA_FILE):
        st.error(
            f"Missing model files ({', '.join(missing)}) and the raw "
            f"dataset `{RAW_DATA_FILE}` isn't present either, so the "
            "app can't train them on the fly. Make sure both the raw "
            "CSV and the generated .pkl/.csv artifacts are committed "
            "to the repo Streamlit deploys from."
        )
        st.stop()

    with st.spinner(
        f"First-time setup: training models from `{RAW_DATA_FILE}` "
        "(missing artifacts detected) — this takes a few seconds..."
    ):
        import feature_engineering
        import train_models

        features = feature_engineering.build_customer_features(RAW_DATA_FILE)
        features.to_csv(feature_engineering.OUT_FILE, index=False)
        train_models.main()
